...
from .models import Vendas, Produto, Vendedor, Despesas
...
def retorna_total_despesas(request):
...
def retorna_total_saida(request):
    total = Despesas.objects.all().aggregate(Sum('valor'))['valor__sum']
    return JsonResponse({'total': total})
